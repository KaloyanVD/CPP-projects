...
float playerXpos = 14.7f;			// Player Start Position
float playerYpos = 5.09f;
...
int main()
{
	...
	while (1)
	{
		...
		if (GetAsyncKeyState((unsigned short)'A') & 0x8000)
			playerAngle -= (speed * 0.75f) * fElapsedTime;
		...
		{
			playerXpos += sinf(playerAngle) * speed * fElapsedTime;;
			playerYpos += cosf(playerAngle) * speed * fElapsedTime;;
			if (map.c_str()[(int)playerXpos * mapWidth + (int)playerYpos] == '#')
			{
				playerXpos -= sinf(playerAngle) * speed * fElapsedTime;;
				playerYpos -= cosf(playerAngle) * speed * fElapsedTime;;
			}
		...
		for (int x = 0; x < screenWidth; x++)
		{
			...
			float fRayAngle = (playerAngle - FOV / 2.0f) + ((float)x / (float)screenWidth) * FOV;
					...
					if (map.c_str()[nTestX * mapWidth + nTestY] == '#')
					{
						...
						for (int tx = 0; tx < 2; tx++)
							for (int ty = 0; ty < 2; ty++)
							{
								...
								float vy = (float)nTestY + ty - playerYpos;
								float vx = (float)nTestX + tx - playerXpos;
		...
		// Display Stats
		swprintf_s(screen, 40, L"X=%3.2f, Y=%3.2f, A=%3.2f FPS=%3.2f ", playerXpos, playerYpos, playerAngle, 1.0f / fElapsedTime);
		...
		screen[((int)playerXpos + 1) * screenWidth + (int)playerYpos] = 'P';
